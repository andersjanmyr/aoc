...
fn main() {
    ...
    let mut tiles = parse_tiles(&data);
    ...
    for mut map in merge_tile_map(&tile_map).orientations() {
        let monster_tails: Vec<(i32, i32)> = map
            .pixels
            .iter()
            .copied()
...
fn merge_tile_map(tile_map: &TileMap) -> Pixels {
    ...
    let (lowx, highx) = set
        .iter()
        .fold((0, 0), |(low, high), &(x, _)| (x.min(low), x.max(high)));
    let (lowy, highy) = set
        .iter()
        .fold((0, 0), |(low, high), &(_, y)| (y.min(low), y.max(high)));
...
fn tile_can_go_here(tile: &Tile, xy: (i32, i32), tile_map: &TileMap) -> bool {
    surrounding(xy)
        .filter(|xy2| tile_map.contains_key(xy2))
        .all(|xy2| {
            ...
            match (xy2.0 - xy.0, xy2.1 - xy.1) {
                ...
                (1, 0) => tile
                    .pixels
                    .rights()
                    .zip(tile_map_tile.pixels.lefts())
                    .all(|(p1, p2)| p1 == p2),
                // tile_map_tile | tile
                (-1, 0) => tile
                    .pixels
                    .lefts()
                    .zip(tile_map_tile.pixels.rights())
                    .all(|(p1, p2)| p1 == p2),
                // tile v tile_map_tile
                (0, 1) => tile
                    .pixels
                    .bottoms()
                    .zip(tile_map_tile.pixels.tops())
                    .all(|(p1, p2)| p1 == p2),
                // tile_map_tile v tile
                (0, -1) => tile
                    .pixels
                    .tops()
                    .zip(tile_map_tile.pixels.bottoms())
                    .all(|(p1, p2)| p1 == p2),
                // unreachable..
                _ => {
                    unreachable!("coords should always be touching")
                }
            }
        })
}
...
impl Tile {
    fn orientations(&self) -> impl Iterator<Item = Tile> {
        ...
        self.pixels
            .orientations()
            .map(move |pixels| Tile { id, pixels })
    }
}
...
impl Pixels {
    ...
    fn orientations(&self) -> impl Iterator<Item = Pixels> {
        ...
        let flip_rots =
            successors(Some(self.flip_horizontal()), move |t| Some(t.rotate_cw())).take(4);
...
fn parse_tiles(s: &str) -> Vec<Tile> {
    ...
    for tile_str in s.split("\n\n") {
        ...
        let id = parse_tile_id(id_line).unwrap();
...
fn parse_tile_id(s: &str) -> Option<i32> {
    let s = s.trim();
    let n = Regex::new("Tile ([0-9]+):").unwrap().captures(s)?[1]
        .parse()
        .ok()?;
    Some(n)
}
...
fn parse_tile_pixels<'a>(lines: impl Iterator<Item = &'a str>) -> HashSet<(i32, i32)> {
    ...
    for (y, line) in lines
        .filter(|l| Regex::new("^[#.]+$").unwrap().is_match(l))
        .enumerate()
    {
